...
function MenuItemCard(props: Props) {
  ...
  const handleAddToCart = async (menuItemId: number) => {
    if (!userData.id) {
      navigate("/login");
      toastNotify("Login first to add to cart", "error");
      return;
    }
    ...
    <div className="col-md-4 col-12 p-4">
      ...
      >
        <div className="card-body pt-2">
          ...
          {/* only show special tag like chef's special top seller etc if there is something in special tag */}
          ...
          {/* to show loading when we click add to cart icon */}
          {isAddingToCart ? (
